...
class BRAIN():
    ...
    def update_ramp(self):
        ...
        #     self.prosimo = 1
        # GOTO_POS["line_1"].centerY = GOTO_POS["line_1"].centerY + self.prosimo*0.01

        self.prosimo = not self.prosimo
        rospy.logerr(self.goto_name)
        if(not(self.goto_name == "line_1")):
            if(self.prosimo):
                plan_the_line[3] = ("GOTO",[self.goto_name+"1" ,True])
                GOTO_POS["cent_1"].centerY = GOTO_POS[self.goto_name+"1"].centerY 
                # GOTO_POS["line_1"] = GOTO_POS[self.goto_name+"1"]
            else:
                plan_the_line[3] = ("GOTO",[self.goto_name+"2" ,True])
                GOTO_POS["cent_1"].centerY = GOTO_POS[self.goto_name+"2"].centerY 
                # GOTO_POS["line_1"] = GOTO_POS[self.goto_name+"2"]

            rospy.logerr(self.goto_name)
            rospy.logerr("-----")
        else:
            GOTO_POS["cent_1"].centerY = GOTO_POS["line_1"].centerY 
